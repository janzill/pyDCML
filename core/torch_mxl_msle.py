...
class TorchMXLMSLE(nn.Module):
    ...
    def __init__(
        ...
        self.zeros_mat = torch.zeros(self.num_menus, self.batch_size, self.num_alternatives).to(
            self.device
        )  # auxiliary matrix for model
        self.alt_ids_cuda = torch.from_numpy(
            self.dcm_spec.alt_id_map[:, np.newaxis]
    ...
    def initialize_parameters(
        ...
        alpha_mu_initial_values = torch.from_numpy(
            np.array(self.dcm_spec.fixed_params_initial_values, dtype=self.numpy_dtype)
        )
        ...
        zeta_mu_initial_values = torch.from_numpy(
            np.array(self.dcm_spec.mixed_params_initial_values, dtype=self.numpy_dtype)
        )
    ...
    def compute_utilities(self, beta_resp, alt_attr, alt_avail, alt_ids):
        ...
        utilities = torch.scatter_add(
            self.zeros_mat, 2, alt_ids.transpose(0, 1), torch.mul(alt_attr.transpose(0, 1), beta_resp)
        )
