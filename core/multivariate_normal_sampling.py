# correlated samples in pytorch: https://github.com/pytorch/botorch/blob/main/botorch/sampling/qmc.py
...
class MultivariateNormalQMCEngine:
    ...
    def __init__(
        ...
        Args:
            ...
            tril: Lower triangular part of cholesky decomp of covariance matrix.
            seed: The seed with which to seed the random number generator of the
                underlying SobolEngine.
            inv_transform: If True, use inverse transform instead of Box-Muller.
        """
        # validate inputs
        if not tril.shape[0] == tril.shape[1]:
            raise ValueError("Covariance matrix is not square.")
        if not mean.shape[0] == tril.shape[0]:
