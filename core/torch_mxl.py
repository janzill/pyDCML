...
class TorchMXL(nn.Module):
    ...
    def elbo(self, alt_attr, context_attr, obs_choices, alt_avail, obs_mask, alt_ids, indices):
        ...
        alpha_mu_initial_values = torch.from_numpy(
            np.array(self.dcm_spec.fixed_params_initial_values, dtype=self.numpy_dtype)).to(self.device)
        ...
        zeta_mu_initial_values = torch.from_numpy(
            np.array(self.dcm_spec.mixed_params_initial_values, dtype=self.numpy_dtype)).to(self.device)
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(0, self.num_resp, self.batch_size):
                ...
                if return_all_results:
                    ...
                    results_this_epoch['L_omega_diag_mu'] = self.L_omega_diag_mu.detach().cpu().numpy().tolist()
                    results_this_epoch['L_omega_offdiag_mu'] = self.L_omega_offdiag_mu.detach().cpu().numpy().tolist()
                    results_this_epoch['L_omega_diag_sigma'] = self.L_omega_diag_sigma.detach().cpu().numpy().tolist()
                    results_this_epoch['L_omega_offdiag_sigma'] = self.L_omega_offdiag_sigma.detach().cpu().numpy().tolist()
                    ...
                    results_this_epoch['alpha_cov_diag'] = self.alpha_cov_diag.detach().cpu().numpy().tolist()
                    results_this_epoch['alpha_cov_offdiag'] = self.alpha_cov_offdiag.detach().cpu().numpy().tolist()
